	...
	class MinHeap(object):
		...
		def sift_up(self, index: int) -> int:
			...
			if parent_idx is not None:
				while i is not None and i > 0 and self.array[self.get_parent(i)] > self.array[i]:
					self.array[self.get_parent(i)], self.array[i] = \
						self.array[i], self.array[self.get_parent(i)]
					i = self.get_parent(i)
		...
		def sift_down(self, index: int) -> int:
			...
			if left_index is not None and left_index < self.size and \
				self.array[left_index] < self.array[min_index]:
				min_index = left_index

			right_index = self.get_right_child(index)
			if right_index is not None and right_index < self.size and \
				self.array[right_index] < self.array[min_index]:
				min_index = right_index

			if index != min_index:
				print('{} {}'.format(index, min_index))
